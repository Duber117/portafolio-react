...
const socialLinks = [
    ...
    {
      href: 'https://www.linkedin.com/in/jhonnier-duberly-parada-valderrama-4248b4237/?jobid=1234',
      icon: <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <section className="section" id="contact">
        <div className="container lg:grid lg:grid-cols-2 lg:items-stretch">
            <div className="mb-12 lg:mb-0 lg:flex lg:flex-col">
                <h2 className="headline-2 lg:max-w-[12ch] reveal-up">{t("Contact me for collaboration")}</h2>
                <p className="text-zinc-400 mt-3 mb-8 max-w-[50ch] lg:max-w-[30ch] reveal-up">{t("Reach out today to discuss your project needs and start collaborating on something amazing!")}</p>
                <div className="flex items-center gap-2 mt-auto">
                    {socialLinks.map(({href, icon}, key)=>(
                        <a href={href} key={key} target="_blank" className="w-12 h-12 grid place-items-center ring-inset ring-2 ring-zinc-50/5 rounded-lg transition-[background-color,color]
                        hover:bg-zinc-50 hover:text-zinc-950 active:bg-zinc-50/80 reveal-up" >{icon}</a>
                    ))}
                </div>
            </div>

            <form action="https://getform.io/f/aroozpnb" method="POST" className="xl:pl-10 2xl:pl-20">
                <div className="md:grid md:items-center md:grid-cols-2 md:gap-2">
                    <div className="mb-4">
                        <label htmlFor="name" className="label reveal-up">{t("Name")}</label>
                        <input type="text" name="name" id="name" autoComplete="name" required placeholder="Duber" className="text-field reveal-up" />
                    </div>

                    <div className="mb-4">
                        <label htmlFor="email" className="label reveal-up">{t("Email")}</label>
                        <input type="text" name="email" id="email" autoComplete="email" required placeholder="dubervalderrama8@gmail.com" className="text-field reveal-up" />
                    </div>

                    <div className="mb-4 md:col-span-2">
                      <label htmlFor="message" className="label reveal-up">{t("Message")}</label>
                    ...
                    <button type="submit" className="btn btn-primary [&]:max-w-full w-full justify-center md:col-span-2 reveal-up">{t("Submit")}</button>
                </div>
            </form>
        </div>
    </section>
